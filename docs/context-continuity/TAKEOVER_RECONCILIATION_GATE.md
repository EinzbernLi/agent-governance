...
Version: 0.3.16
Status: Accepted takeover/continuity owner (preserves the 0.3.13 barrier)
...
Durable authority resolves by semantic role and exact revision, never by naive
comment recency or open/closed status alone:

```text
accepted main/release baseline
-> canonical Lead sink continuity
-> exact frozen Task revision
-> exclude protected/forbidden older work
```

A later exact frozen Task revision supersedes a stale proposal/body status for
execution. Older protected work cannot become the resume target merely because
it remains open. This generic precedence does not change the terminal
`AWAIT_OWNER_CONTINUE` barrier.
...
After deriving `RESUME_TARGET`, publish a compact current-state/recommended-action
summary and wait. Formal startup-card and launcher presentation are owned only by
`docs/task-package/TASK_PACKAGE_SPEC.md`; this takeover protocol does not mirror
that contract. An unreadable, missing, drifted or contradictory Task still fails
closed rather than being reconstructed from chat.
...
Expected: presentation is checked against the Task Package owner; this takeover
surface contributes no second launcher definition.
