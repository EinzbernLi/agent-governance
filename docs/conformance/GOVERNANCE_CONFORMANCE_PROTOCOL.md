...
## 7. Consolidation contracts

The policy carries derived verification metadata for the shared-kernel owner
map, delta-only runtime profiles, override allowlist, deterministic cold-path
triggers, durable authority precedence, launcher presentation and simplicity
metrics. These entries point to existing canonical owners; they are not
governance semantics or a composition language of their own.

Unknown semantic-owner keys, runtime override fields, cold-path categories or
precedence facts fail closed. Runtime profiles may vary only execution economy,
otherwise-compliant direct/delegate preference, delegable preparation,
wait/transport preference and launcher hint. They cannot alter Task, Owner,
permission, safety, Lead, validation, acceptance, takeover, pin or conformance
meaning.

The same policy validation checks execution placement and native-agent Worker
delegation. Runtime mode is capability-defined rather than client-brand-defined:
work requiring local materialization prefers a proven local-capable Agent
surface, while remote-only work may prefer a non-native interactive surface;
higher-precedence constraints still win. A native-subagent-capable Worker may
use qualified native children when allowed, with retained parent accountability,
disjoint write ownership, no independent-validation substitution and no default
child recursion. Compatibility profile names such as `codex_native_subagents`
do not make a client label capability proof.

Cold paths resolve from durable triggers to exact owners without discovery,
registry, loader or daemon. Exact frozen Task revisions outrank stale proposal
status by semantic role/revision, while protected older open work is excluded;
the takeover result still stops at `AWAIT_OWNER_CONTINUE`.

The evaluator reports before/after hot-path size, duplicate-owner count,
full-core runtime-profile count, collapsed surfaces and net authority direction.
A missed frozen simplification target is not healthy evidence.

## 8. Thin launcher and optional Console

Formal presentation is owned by `docs/task-package/TASK_PACKAGE_SPEC.md` and is
validated here without being redefined. The full durable Task remains authority.

The optional Governance Console may display conformance Results as a derived
read model only. It is not authority and is not a prerequisite for this
protocol or for ordinary Core adoption.

## 9. Qualification

`tests/test_governance_conformance.py` covers the 0.3.16 Q1-Q15 consolidation
replays, the presentation-drift addendum, representative 0.3.13-0.3.15 behavior,
determinism, read-only/offline behavior, duplicate-key/unknown-fact fail-closed
handling and the exact maximum write-surface declaration.
