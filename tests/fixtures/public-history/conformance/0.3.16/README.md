...
Canonical ownership is machine-checked by `config/CONFORMANCE_POLICY.yaml`.
...
收到后续明确执行指令后，按 `config/DISPATCH_POLICY.yaml` 先解析 execution surface，再解析 economy/route：需要 local checkout/files/software/tests/artifacts 的工作默认 Codex，remote-only work 默认 Web；higher-precedence role/model/independence/safety/capability 仍可覆盖。README 不复制完整规则。
...
启动卡和 launcher 的 exact presentation contract 只由
`docs/task-package/TASK_PACKAGE_SPEC.md` 定义。Delegated lifecycle、observation 和
nested delegation 只由 `config/DISPATCH_POLICY.yaml` 定义。
...
- `docs/protocol/DISPATCH_ROUTING_PROTOCOL.md`（非权威说明）
...
Codex Worker 在 native capability 已证明时默认可选用 qualified children；Web/
generic Worker 默认 false。Parent 始终保留 scope/write/integration/terminal Result
accountability，child review 不等于 independent validation。
...
The shared runtime-neutral kernel is a reference manifest in
`config/CONFORMANCE_POLICY.yaml`; each invariant resolves to one canonical owner.
README examples and summaries do not redefine those rules.

## Canonical Files

Use `config/CONFORMANCE_POLICY.yaml:semantic_authority.owners` as the
machine-checkable owner map. It points to the canonical files for continuity,
Task, dispatch/runtime profiles, coordination, model routing, acceptance,
currency, conformance/local actions, optional LPRL, launcher presentation and
release identity. Conditional modules are loaded only when their durable trigger
is present.
