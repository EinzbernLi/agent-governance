...
## 13. Conformance Evaluation Is Not Adoption

The offline evaluator may report `update_available: true` while the project
remains `CONFORMANT` at an older compatible independent pin. That advisory does
not fetch latest, adopt, edit `GOVERNANCE_LOCK`, or repin a module.

Missing, contradictory or incompatible pin facts return
`RECONCILIATION_REQUIRED` or `BLOCKED`; they never trigger follow-main. Run the
gate when governance is adopted/updated or an independent module is repinned,
using exact accepted refs supplied by the project.
