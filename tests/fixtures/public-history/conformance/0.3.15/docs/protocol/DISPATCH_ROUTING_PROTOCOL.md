...
本协议是普通任务唯一的 Execution Economy + Dispatch 决策规范。它不重新决定模型资格，不复制 Task scope，也不追踪普通任务内部每一级 Agent 身份。
...
普通 Task 的 assigned Worker/Validator 默认不得继续创建 internal child。只有 Task 或 Lead 明确 opt-in，且父执行 session 仍对 scope、权限、forbidden boundary、substantive-evidence isolation 和 Result 负责时，才可在自身 bounded scope 内使用平台原生 internal subagent；这不是 Lead dispatch，也不会形成新的治理层。

Opt-in 必须同时证明：subproblem bounded 且 independently understandable；存在 material specialization、context isolation 或 elapsed-time benefit；write/resource ownership 明确；无 unsafe shared-write conflict；scope、permissions、forbidden boundary、evidence boundary 与 result sink 均不扩张。禁止 convenience-only recursion、speculative fan-out、purposeless duplicate execution 和 uncontrolled recursive agent trees。独立验证、明确返工、失败/中止恢复或 justified executor replacement 等 distinct purpose 不视为 purposeless duplicate。
