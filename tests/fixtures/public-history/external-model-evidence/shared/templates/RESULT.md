...
task_revision_binding:
  executed_exact_task_ref: ""  # exact activated revision actually executed
  reanchor_used: false
  reanchor_from_ref: null
  reanchor_to_ref: null
  reanchor_evidence_ref: null
...
`task_ref` identifies the Task carried by this Result;
`task_revision_binding.executed_exact_task_ref` records the immutable running
contract actually executed. When `reanchor_used` is false, all re-anchor refs
remain null. When true, from/to/evidence refs are required and must identify the
explicit authorized re-anchor that changed the binding.
